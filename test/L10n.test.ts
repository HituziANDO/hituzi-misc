...
describe('L10n', () => {
  ...
  it('should register a dictionary', () => {
    expect(L10n['dict']).toEqual(dict);
    expect(L10n['lang']).toBe('ja');
  });

  it('should get a localized string', () => {
  ...
  it('should get a localized string using l10n helper function', () => {
    expect(l10n('hello')).toBe('こんにちは');
    expect(l10n('bye', undefined, 'en')).toBe('Goodbye');
    expect(l10n('unknown-key')).toBe('unknown-key');
  });

  it('should format a localized string using l10n helper function', () => {
